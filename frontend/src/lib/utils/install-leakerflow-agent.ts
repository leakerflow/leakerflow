...
async function installLeakerFlowForNewUser(userId: string) {
  try {
    ...
    if (!adminApiKey) {
      console.error('KORTIX_ADMIN_API_KEY not configured - cannot install LeakerFlow agent');
      return;
    }
    
    const response = await fetch(`${backendUrl}/admin/leakerflow-agents/install-user/${userId}`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      console.error(`Failed to install LeakerFlow agent for user:`, errorData);
      return false;
    }
  } catch (error) {
    console.error('Error installing LeakerFlow agent for new user:', error);
    return false;
  }
}
...
export async function checkAndInstallLeakerFlowAgent(userId: string, userCreatedAt: string) {
  ...
  if (userCreatedDate > tenMinutesAgo) {
    const installKey = `leakerflow-install-attempted-${userId}`;
