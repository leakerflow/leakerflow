...
export function useMarketplaceTemplates(params?: {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AgentTemplate[]> => {
      ...
      if (params?.is_leakerflow_team !== undefined) searchParams.set('is_leakerflow_team', params.is_leakerflow_team.toString());
...
export function useLeakerFlowTeamTemplates() {
  return useMarketplaceTemplates({
    is_leakerflow_team: true,
    limit: 10
  });
}
