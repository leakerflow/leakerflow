...
from leakerflow import Leakerflow, MCPTools
from leakerflow.utils import print_stream
...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the Leaker Flow instance
    mcp_tools = MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running Leaker Flow locally, we can use the local URL
        "Leaker Flow",
        allowed_tools=["get_wind_direction"],
    )
    await mcp_tools.initialize()

    leakerflow_client = Leakerflow(
        os.getenv("LEAKERFLOW_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await leakerflow_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await leakerflow_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await leakerflow_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await leakerflow_client.Thread.get(thread_id)
