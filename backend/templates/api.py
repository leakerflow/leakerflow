...
class TemplateResponse(BaseModel):
    ...
    tags: List[str]
    is_public: bool
    is_leakerflow_team: Optional[bool] = False
...
async def get_marketplace_templates(
    ...
    is_leakerflow_team: Optional[bool] = Query(None, description="Filter for LeakerFlow team templates")
):
    try:
        logger.info(
            f"Fetching marketplace templates with filters - "
            f"limit: {limit}, offset: {offset}, search: {search}, "
            f"tags: {tags}, is_leakerflow_team: {is_leakerflow_team}"
        )
        ...
        templates = await template_service.get_public_templates(
            is_leakerflow_team=is_leakerflow_team,
            limit=limit,
            offset=offset,
            search=search,
            tags=tag_list
