...
-- Create RPC function to find Leaker Flow default agent for an account
CREATE OR REPLACE FUNCTION find_leakerflow_default_agent_for_account(p_account_id UUID)
RETURNS TABLE (
    agent_id UUID,
    account_id UUID,
    name VARCHAR(255),
...
BEGIN
    ...
    AND COALESCE((a.metadata->>'is_leakerflow_default')::boolean, false) = true
    ORDER BY a.created_at DESC
    LIMIT 1;
END;
$$;

-- Create function to get all Leaker Flow default agents
CREATE OR REPLACE FUNCTION get_all_leakerflow_default_agents()
RETURNS TABLE (
    agent_id UUID,
    account_id UUID,
    name VARCHAR(255),
...
BEGIN
    ...
    FROM agents a
    WHERE COALESCE((a.metadata->>'is_leakerflow_default')::boolean, false) = true
    ORDER BY a.created_at DESC;
END;
$$;

-- Create function to count agents by management version
CREATE OR REPLACE FUNCTION count_leakerflow_agents_by_version(p_version TEXT)
RETURNS INTEGER
SECURITY DEFINER
LANGUAGE plpgsql
AS $$
...
-- Create function to get Leaker Flow default agent statistics
CREATE OR REPLACE FUNCTION get_leakerflow_default_agent_stats()
RETURNS TABLE (
...
BEGIN
    ...
    -- Get active count (all Leaker Flow agents are considered active)
...
CREATE OR REPLACE FUNCTION find_leakerflow_agents_needing_update(p_target_version TEXT)
RETURNS TABLE (
    agent_id UUID,
    account_id UUID,
    name VARCHAR(255),
...
GRANT EXECUTE ON FUNCTION find_leakerflow_default_agent_for_account(UUID) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_all_leakerflow_default_agents() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION count_leakerflow_agents_by_version(TEXT) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION get_leakerflow_default_agent_stats() TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION find_leakerflow_agents_needing_update(TEXT) TO authenticated, service_role;
