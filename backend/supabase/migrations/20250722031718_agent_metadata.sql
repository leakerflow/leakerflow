...
CREATE INDEX IF NOT EXISTS idx_agents_leakerflow_default 
ON agents((metadata->>'is_leakerflow_default')) 
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_agents_leakerflow_default_unique 
ON agents(account_id) 
WHERE metadata->>'is_leakerflow_default' = 'true';

COMMENT ON COLUMN agents.metadata IS 'Stores additional agent metadata including:
- is_leakerflow_default: boolean - Whether this is the official Leaker Flow default agent
- centrally_managed: boolean - Whether this agent is managed centrally by Leaker Flow
...
CREATE OR REPLACE FUNCTION is_leakerflow_default_agent(agent_row agents)
RETURNS boolean
LANGUAGE plpgsql
IMMUTABLE
AS $$
BEGIN
    RETURN COALESCE((agent_row.metadata->>'is_leakerflow_default')::boolean, false);
END;
$$;
...
GRANT EXECUTE ON FUNCTION is_leakerflow_default_agent(agents) TO authenticated, service_role;
...
CREATE POLICY agents_update_own ON agents
    ...
    USING (
        ...
        AND (
            NOT COALESCE((metadata->>'is_leakerflow_default')::boolean, false)
            OR 
            (
                COALESCE((metadata->>'is_leakerflow_default')::boolean, false) = true
            )
        )
    );
...
CREATE POLICY agents_delete_own ON agents
    ...
    USING (
        ...
        AND NOT COALESCE((metadata->>'is_leakerflow_default')::boolean, false)
    );
