...
class LeakerflowManagerCLI:
    ...
    async def cleanup_command(self):
        ...
        try:
            ...
            print_info("💡 Now run: python leakerflow_manager.py install")
    ...
    async def status_command(self):
        ...
        try:
            ...
            leakerflow_agents_result = await client.table('agents').select(
                'agent_id'
            ).eq('metadata->>is_leakerflow_default', 'true').execute()
            
            leakerflow_agent_ids = [a['agent_id'] for a in (leakerflow_agents_result.data or [])]
            
            if leakerflow_agent_ids:
                leakerflow_versions_result = await client.table('agent_versions').select(
                    'version_id', count='exact'
                ).in_('agent_id', leakerflow_agent_ids).execute()
                total_leakerflow_versions = leakerflow_versions_result.count or 0
            else:
                total_leakerflow_versions = 0
            ...
            print_info(f"Agent versions created: {total_leakerflow_versions}")
            
            if broken_count > 0:
                print_warning(f"Broken agents (no version): {broken_count}")
                print_info("💡 Run: python leakerflow_manager.py cleanup")
            else:
            ...
            if remaining > 0:
                ...
                print_info("💡 Run: python leakerflow_manager.py install")
            else:
    ...
    async def install_command(self, batch_size: int = 100):
        ...
        try:
            ...
            existing_agents = await self.sync_service.repository.find_all_leakerflow_agents()
    ...
    async def repair_command(self):
        ...
        try:
            ...
            unified_config = {
                ...
                "tools": {
                    "agentpress": LeakerFlowConfig.DEFAULT_TOOLS
                }
            }
            ...
            orphaned = await repo.find_orphaned_leakerflow_agents()
            if not orphaned:
